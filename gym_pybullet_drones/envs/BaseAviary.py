...
class BaseAviary(gym.Env):
    ...
    def step(self,
        ...
        #### Save, preprocess, and clip the action to the max. RPM #
            ...
            if self.ACT_TYPE != ActionType.XY:
                ...
                clipped_action = np.atleast_2d(action-1)
        ...
        for _ in range(self.AGGR_PHY_STEPS):
            ...
            self.last_clipped_action[0][0:len(clipped_action[0])] = clipped_action[0]
        #### Update and store the drones kinematic information #####
        self._updateAndStoreKinematicInformation()
    ...
    def _groundEffect(self,
        ...
        #### a more realistic model accounting for the drone's
    ...
    def _dynamics(self,
        ...
        # print("test ", pos, input, self.step_counter)
        if self.ACT_TYPE == ActionType.XY:
            # print(input[0:2],np.round(pos,2))
            envSize = 2.0
            k = 0.1 #envSize * self.TIMESTEP
            pos = pos + np.array([k*input[0], k*input[1], 0])
            # print("test", pos)
        else:
            rpm = input
        ...
        self._computeObstacleDynamics()
        if self.env_obs_dy:
            # print("updated",self.env_obs_pos)
            p.resetBasePositionAndOrientation(self.env_obs, 
                                            self.env_obs_pos,
                                            self.env_obs_ori,
                                            physicsClientId=self.CLIENT)
        # cubePos, cubeOrn = p.getBasePositionAndOrientation(self.dy_obs)
        # print("test",self.DRONE_IDS,"test",cubePos)
    ...
    def _addObstacles(self):
        ...
        p.loadURDF(os.path.join(pybullet_data.getDataPath(),"r2d2.urdf"),[0,0,0])
