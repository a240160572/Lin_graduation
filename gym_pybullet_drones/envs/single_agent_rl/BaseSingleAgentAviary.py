...
from gym_pybullet_drones.envs.BaseAviary import DroneModel, Physics, ImageType, BaseAviary, ObservationType, ActionType
...
class BaseSingleAgentAviary(BaseAviary):
    ...
    def __init__(self,
        ...
        dynamics_attributes = True if act in [ActionType.XY, ActionType.RP, ActionType.DYN, ActionType.ONE_D_DYN] else False
        self.OBS_TYPE = obs
        self.ACT_TYPE = act
    ...
    def _actionSpace(self):
        ...
        """
        if self.ACT_TYPE == ActionType.XY:
        ...
        if self.ACT_TYPE == ActionType.TUN:
            ...
            size = 4
        elif self.ACT_TYPE in [ActionType.RP]:
            size = 2
        ...
        action_space = spaces.Box(low=-.1*np.ones(size),
                          high=.1*np.ones(size),
                          dtype=np.float32
                        #   low=np.array([ -0.2, -0.2, -0.2, -0.2]),
                        #   high=np.array([0.2, 0.2, 0.2, 0.2]),
                          )
    ...
    def _preprocessAction(self,
        ...
        elif self.ACT_TYPE == ActionType.XY:
            return np.zeros((4,))
        elif self.ACT_TYPE == ActionType.RP:
            quat = self.quat[self.NUM_DRONES-1,:]
            rotation = np.array(p.getMatrixFromQuaternion(quat)).reshape(3, 3)
            thrust_to_hover = np.dot(np.linalg.inv(rotation),np.array([0,0,self.GRAVITY]))
            # print(thrust_to_hover)
            return nnlsRPM(thrust=thrust_to_hover[2],
                           x_torque=(0.05*self.MAX_XY_TORQUE*action[0]),
                           y_torque=(0.05*self.MAX_XY_TORQUE*action[1]),
                           z_torque=0,
                           counter=self.step_counter,
                           max_thrust=self.MAX_THRUST,
                           max_xy_torque=self.MAX_XY_TORQUE,
    ...
    def _observationSpace(self):
        ...
        elif self.OBS_TYPE == ObservationType.XY:
    ...
    def _computeObs(self):
        ...
        if self.OBS_TYPE == ObservationType.RGB:
            ...
            obs = self._clipAndNormalizeState(self._getDroneStateVector(0))
            # print(self._getDroneStateVector(0)[0:3], obs[0:3])
            ############################################################
            #### OBS OF SIZE 20 (WITH QUATERNION AND RPMS)
            # return obs
            ############################################################
