...
if __name__ == "__main__":
    ...
    if ARGS.exp.split("-")[4] == 'rpm':
        ACT = ActionType.RPM
    elif ARGS.exp.split("-")[4] == 'rp':
        ACT = ActionType.RP
    elif ARGS.exp.split("-")[4] == 'xy':
        ACT = ActionType.XY
    ...
    p.resetDebugVisualizerCamera(cameraDistance=0.7,
                                 cameraYaw=250,
                                 cameraPitch=-20,
                                 cameraTargetPosition=[-1, -0.2, 1],
                                 physicsClientId=test_env.CLIENT
                                 )

    for i in range(6*int(test_env.SIM_FREQ/test_env.AGGR_PHY_STEPS)): # Up to 6''
        action, _states = model.predict(obs,
                                        deterministic=1 # True # OPTIONAL 'deterministic=False'
                                        )
        # print("time: ", i/test_env.SIM_FREQ, "action", action)
