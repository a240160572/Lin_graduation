...
#      Animation for a single scenatio
...
for log_num in track_list:
    log_data = np.load(ARGS.exp+'eva_result_{}.npy'.format(log_num),allow_pickle='TRUE').item()
    pybullet.connect(pybullet.GUI, options= "--mp4={}\"test_{}.mp4\" --mp4fps=100".format(ARGS.exp, log_num))
    ...
    if "obs_pos" in log_data:
        ...
        #     vars()["obs"+str(p)]=pybullet.loadURDF(str(Path().absolute().parents[1])+"/gym_pybullet_drones/assets/obs.urdf", np.concatenate((log_data["obs_pos"][p], 0.5), axis=None))
    ...
    for i in range(75):
        ...
        if i >= len(log_data["obs"]):
            ...
            for j in range(10):
                pybullet.resetBasePositionAndOrientation(drone, [drone_x[j], drone_y[j], 0.75],pybullet.getQuaternionFromEuler([0,0,0]))
                if np.shape(np.atleast_2d(log_data["obs_pos"]))[0] == 1:
