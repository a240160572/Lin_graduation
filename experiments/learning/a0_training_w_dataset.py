...
class planning(gym.Env):
  ...
  def generate_ini_observation(self):
    ...
    if self.vel_obs == True:
      self.ini_observation = np.concatenate((self.pos, self.goal, self.obstacle_pos, self.obstacle_pos - self.obstacle_pos), 
                                  axis=None)/self.env_size 
  ...
  def step(self, action):
    ...
    ########### Update observation #############
    observation = np.concatenate((self.pos, self.goal , self.obstacle_pos, self.obstacle_pos - old_obstacle_pos), 
                                      axis=None)/self.env_size 
  ...
  def compute_reward(self):
    ...
    if max(abs(self.pos)) >= 1.5*self.env_size: #1.5*self.env_size:                         # hit the boundary 
...
if __name__ == "__main__":
  ...
  parser.add_argument('--vel',                       default="False",      action='store_true')
  ...
  if ARGS.exp is None:
    if ARGS.note is not None:
      models_dir = str(Path().absolute()) + "/model_logs/"+datetime.now().strftime("%m.%d_%H.%M")+"_t_" + ARGS.note
    else:
      models_dir = str(Path().absolute()) + "/model_logs/"+datetime.now().strftime("%m.%d_%H.%M")+"_t"
  ...
  alg_list = ["PPO","A2C","TRPO","RPPO","SAC"] #RPPO - recurrent PPO
  ...
  if ARGS.exp is not None:
    ...
    for ind, i in enumerate(model_index):
      ...
      model.save(alg_dir+'/success_{}_{}.zip'.format(alg_list[i], np.int_(model._total_timesteps/model.n_steps)))
      np.save(alg_dir+"scenario_{}.npy".format(env.scenatio_num), np.array(env.scenatio_num))
