...
class planning(gym.Env):
  ...
  def __init__(self, dataset):
    ...
    self.action_space = spaces.Box(low= -1 * np.ones(2),
                                   high= np.ones(2),
                                   dtype= np.float32
                                  )
  ...
  def compute_reward(self):
    ...
    if max(abs(self.pos)) >= 1.5*self.env_size:                         # hit the boundary 
...
if __name__ == "__main__":
  ...
  if ARGS.exp is None:
    if ARGS.note is not None:
      models_dir = str(Path().absolute()) + "/model_logs/"+datetime.now().strftime("%m.%d_%H.%M")+"_2act_" + ARGS.note
    else:
      models_dir = str(Path().absolute()) + "/model_logs/"+datetime.now().strftime("%m.%d_%H.%M")+"_2act"
