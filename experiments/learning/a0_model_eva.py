...
def plot_traj(log):
    ...
    traj = plt.plot(log["obs"][:,0], log["obs"][:,1])
...
if __name__ == "__main__":
  ...
  for i in algo_index:
    ...
    obs_list = {"ini_pos": env.ini_pos,
              "goal_pos": env.goal,
              "obs_pos": env.obstacle_pos,
              "obs": np.round(np.array([observation*env.env_size]),2)}

    # print(len(obs_list["obs_pos"]))
    ...
    ax.axis('square')
    plot_traj(obs_list)
    fig.savefig(models_dir+'/{}/{}_{}.png'.format(filenames[i], algo_type[0], ARGS.model), dpi=fig.dpi)

    np.save(models_dir+'/{}/subs_log/eva_result_{}_0.npy'.format(filenames[i], ARGS.model), obs_list)
    ...
    if ARGS.subs == True:
      for j in range(plot_num*plot_num):
          plt.subplot(plot_num, plot_num, int(j+1))
          ax = plt.gca()
          # ax.axis('square')
          ...
          for step in range(env.max_step):
              ...
              if done:
                  break 
          plot = plot_traj(obs_list)

          np.save(models_dir+'/{}/subs_log/eva_result_{}_{}.npy'.format(filenames[i], ARGS.model, j+1), obs_list)
          # if j == 11:
      ...
      fig.savefig(models_dir+'/{}/subs_{}_{}.eps'.format(filenames[i], algo_type[0], ARGS.model), dpi=fig.dpi,format='eps')
